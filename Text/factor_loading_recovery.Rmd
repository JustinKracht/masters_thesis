...
where $r$ is the true correlation between $y_1^*$ and $y_2^*$ that is estimated by the tetrachoric correlation, $\hat{r}$. To compute the tetrachoric correlation, a $2 \times 2$ contingency table is first created using $y_1$ and $y_2$ as described in @brown1977mean. If any of the cell frequencies in the contingency table are zero, it is common to replace them with 0.5 and adjust the other elements in order to leave the marginal sums unchanged [@brown1997mean]. The proportions of correct responses for $y_1^*$ and $y_2^*$ are represented by the marginals $p_1$ and $p_2$. The standard normal deviate thresholds *h* and *k* used to dichotomize $y_1$ and $y_2$ are then defined by $F(h) = p_1$ and $F(k) = p_2$ where $F(z)$ is the area under the standard normal curve from 0 to $\infty$ [@divgi1979]. Given \@ref(eq:bivariate-normal), *h*, and *k*, the likelihood function can be written as
...
where $p_{11}$ is the proportion of correct responses on both $y_1^*$ and $y_2^*$ and $L^\prime(h,k,\hat{r}_i)$ is the first derivative of \@ref(eq:likelihood-fn) [@divgi1979]. Iteration continues until convergence is acheived (when $\hat{r}_{i+1} - \hat{r}_i < \delta$ for some small value of delta) or until some maximum number of iterations occur. For $p$ dichotomous variables, the matrix $\hat{\mathbf{R}}_{p \times p}$ with a unit diagonal and off-diagonal elements $\hat{r}_{jk}, \: j,k \in \{1, \dots p \}$ is called the tetrachoric correlation matrix.
