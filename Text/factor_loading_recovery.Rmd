...
Although the common factor analysis model represented in \@ref(eq:cfa) is often useful, many authors have remarked that it constitutes an oversimplification of the complex processes that generate real, observed data [@cudeck1991model; @maccallum1991representing; @maccallum2001]. Tucker, Koopman, and Linn [-@tucker1969] suggested that the lack-of-fit between the common factor model and the complex processes underlying real data could be represented by modeling a large number of minor common factors of small effect. The model Tucker et al. (1969) proposed can be written as:
\begin{equation}
...
Each data set in the simulation was generated as follows. First, a model-implied population correlation matrix, $\Rpop$, was obtained using
$$
\Rpop = \mathbf{F} \mathbf{\Phi} \mathbf{F}^{\prime} + \mathbf{\Theta}^2 + \mathbf{WW}^{\prime}.
(\#eq:sim-mod)
$$
Here, $\mathbf{F}$ denotes a $p \times m$ matrix of major factor loadings with simple structure such that each factor had exactly $p/m$ salient loadings (fixed at the value indicated by the level of Loading) and all other loadings fixed at zero. Because only orthogonal models were considered in this study, the factor correlation matrix $\mathbf{\Phi}$ was an $m \times m$ identity matrix. The $p \times q$ matrix of minor common factor loadings, $\mathbf{W}$, was constructed in multiple steps. First, a $p \times q$ provisional matrix, $\mathbf{W}^*$, was generated such that the $i$th column of $\mathbf{W}^*$ consisted of $p$ independent samples from $N(0, ((1 - \epsilon)^{i-1})^2)$ where $\epsilon \in [0,1]$ was a user-specified constant. The value of $\epsilon$ determined how the minor common factor (error) variance was distributed. Values of $\epsilon$ close to zero resulted in the error variance being spread relatively equally among the minor common factors. Values of $\epsilon$ close to one resulted in error variance primarily being distributed to the first minor factor, with the remaining variance distributed to the other minor factors in a decreasing geometric sequence. To ensure that the minor common factors accounted for the proportion of uniqueness variance ($U_p$), $\mathbf{W}^*$ was scaled to create $\mathbf{W}$. This was accomplished in several steps. First, a diagonal matrix $\mathbf{U}_{p \times p} = \Diag(\mathbf{F}\mathbf{F}^\prime)$, where $\Diag(\mathbf{F}\mathbf{F}^\prime)$ is to be read as the diagonal matrix formed from the diagonal entries in $\mathbf{F}\mathbf{F}^\prime$. Next, another diagonal matrix $\mathbf{Z}_{p \times p}$ was created with diagonal elements $z_i = \sigma^2_{ME} / U_p$, $i \in \{1, \dots, p\}$. Then, $\mathbf{W} = (\Diag(\mathbf{W}^* \mathbf{W}^{*\prime})^{-1} \mathbf{Z})^{1/2} \mathbf{W}$.
