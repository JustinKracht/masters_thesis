...
abstract: |
  Researchers commonly use tetrachoric correlation matrices in item factor analysis. Unfortunately, tetrachoric correlation matrices are often non-positive definite (NPD; i.e., having one or more negative eigenvalues). These NPD correlation matrices are necessarily biased estimates of their corresponding population correlation matrices because correlation matrices are definitionally PSD. When used in procedures such as factor analysis, biased NPD tetrachoric correlation matrices may result in correspondingly biased parameter estimates (i.e., factor loadings). Matrix smoothing algorithms attempt to remedy this problem by finding a PSD correlation matrix that is close, in some sense, to an NPD correlation matrix. However, little research has been done to investigate the effectiveness of matrix smoothing for recovering population correlation matrices or for improving factor loading estimates when used in factor analysis. In the present simulation study, a large number of NPD tetrachoric correlation matrices were calculated from realistic simulated binary data sets. Three widely-used matrix smoothing algorithms---the Higham (2002), Bentler-Yuan (2011), and Knol-Berger algorithms (1991)---were then applied to the NPD tetrachoric correlation matrices and factor extraction was conducted on the smoothed and unsmoothed correlation matrices. The results of this study show that smoothed matrices were better estimates of their population counterparts compared to unsmoothed NPD correlation matrices. However, using smoothed compared to unsmoothed NPD correlation matrices for item factor analysis did not meaningfully improve factor loading recovery.
...
where $\mathbf{P}$ is a $p \times p$ population correlation matrix for $p$ observed (standardized) variables, $\mathbf{F}$ is a $p \times m$ factor loading matrix for $m$ common factors, $\mathbf{\Phi}$ is an $m \times m$ matrix of correlations between the $m$ common factors, and $\mathbf{\Theta}^2$ is a $p \times p$ diagonal matrix containing the unique variances.
...
where $\mathbf{W}$ is a $p \times q$ matrix containing factor loadings for the $q \gg m$ minor factors [@briggs2003, p. 32]. Given our expectation that the common factor model is not a perfect representation of any real-world data-generating process we might wish to represent, the model of @tucker1969 should be preferred for simulating more realistic data [@hong1999].
...
The unweighted least-squares factor extraction method [ULS; also known as ordinary least-squares or "minres", @comrey1962minimum] seeks to minimize to minimize the sum of squared differences between the sample correlation matrix, $\mathbf{R}$, and $\hat{\mathbf{P}}$, the correlation matrix implied by the estimated factor model such that

\begin{equation}
\hat{\mathbf{P}} = \hat{\mathbf{F}}\hat{\mathbf{F}}^\prime + \hat{\mathbf{\Theta}}^2.
(\#eq:cfa)
\end{equation}

where $\hat{\mathbf{F}}$ is a $p \times m$ matrix of estimated factor loadings for $m$ common factors and $\hat{\mathbf{\Theta}}^2$ is a $p \times p$ diagonal matrix containing the unique variance estimates [@briggs2003]. The ULS discrepancy function can then be written as

\begin{equation}
F_{ULS}(\mathbf{R}, \hat{\mathbf{P}}) = \frac{1}{2} \tr \left[ (\mathbf{R} - \hat{\mathbf{P}})^2 \right],
(\#eq:common-factor-model-error)
\end{equation}
...
To do this, I calculated tetrachoric correlation matrices for a large number of simulated, binary data sets with known factor structure. I then applied the Higham (2002), Bentler-Yuan (2011), and Knol-Berger (1991) matrix smoothing algorithms to NPD tetrachoric correlation matrices to produce smoothed, PSD correlation matrices. Next, I compared these smoothed matrices (and the original NPD matrices) with the model-implied population correlation matrices to evaluate the relative effectiveness of each smoothing algorithm at recovering the $\Rpop$. 
